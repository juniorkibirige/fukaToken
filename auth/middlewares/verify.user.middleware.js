...
exports.hasAuthValidFields = (req, res, next) => {
    let errors = Array()
    if (req.body) {
        !req.body.email && !req.body.phoneNumber ? errors.push('Missing email/phone field!') : null
        !req.body.email ? !req.body.phoneNumber ? errors.push('Missing email/phone field!') : null : null
...
exports.isPasswordAndUserMatch = (req, res, next) => {
    ...
    if (req.body.email !== undefined) {
        ...
        UserModel.findByPhone(req.body.phoneNumber)
        .then(user => {
            if (!user[0]) {
                res.status(404).send({ success: false, errors: ['User doesn\'t exist!'] })
            } else {
