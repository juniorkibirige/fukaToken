...
exports.create = (req, res) => {
    UserModel.findByEmail(req.body.email).then((result) => {
        if (result.length === 0) {
            UserModel.findByPhone(req.body.phoneNumber).then(data => {
                if (data.length === 0) {
                    ...
                    res.status(409).send({ emsg: "User Phone Number already in use" })
                }
            })
        } else {
            res.status(409).send({ emsg: "User Email already in use" })
        }
    })
}
...
exports.getByPhone = (req, res) => {
    UserModel.findByPhone(req.params.phone).then(result => {
        if (result.length == 0) res.status(404).send({success: false});
        else res.status(201);
    })
}
